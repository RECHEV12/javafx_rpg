...
public class ShopController {
    ...
    public void showItem() {
        label.setText("소지금 : " + user.getMoney());
        ItemDB itemDB = new ItemDB();
        ArrayList<Item> items = itemDB.getItemList();
        ArrayList<Item> userItemList = user.getItemsList();
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            price.setText("판매가격 : " + (item.getPrice()*2));
        ...
        for (int i = 0; i < userItemList.size(); i++) {
            ...
            useBtn.setPrefWidth(fourthBox.getPrefWidth());
            useBtn.setAlignment(Pos.CENTER);
            useBtn.setOnAction(event -> sellItem(item));
    ...
    public void showItemPopup(Item item) {
        try {
            ...
            BuyItemController bic = fxmlLoader.getController();
            bic.showUpStat(item);
            Stage popupStage = new Stage();
            popupStage.initModality(Modality.APPLICATION_MODAL);
            popupStage.setTitle("buyItem");
